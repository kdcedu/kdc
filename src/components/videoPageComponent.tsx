...
import { lessonVideos, videos } from "@/constant/videos";
import {
  BackwardOutlined,
  CheckCircleFilled,
  CloseCircleFilled,
...
interface VideoPageComponentProps {
  isLesson?: boolean;
}

export default function VideoPageComponent({
  isLesson,
}: VideoPageComponentProps) {
  ...
  const video = isLesson
    ? lessonVideos.find((item) => item.id === id)
              ...
              {!isNoShare && (
                <CatConversation
                  icon={
                    <AudioPlayer
                      onClick={handlePause}
                      src="/audios/share.mp3"
                    />
                  }
                >
                  Mình rất vui vì bạn đã chia sẻ cảm xúc với mình. Vậy bạn có
                  muốn chia sẻ video này đến bạn bè không?
                </CatConversation>
              )}
              ...
              {isNoShare && video?.isGood && (
                <CatConversation
                  icon={
                    <AudioPlayer
                      onClick={handlePause}
                      src="/audios/noshare.mp3"
                    />
                  }
                >
                  Không sao cả, mình xem video khác thôi. Lần sau, có nội dung
                  hay, bạn hãy chia sẻ nhé!
                </CatConversation>
              )}
              ...
              {isNoShare && !video?.isGood && (
                <CatConversation
                  icon={
                    <AudioPlayer
                      onClick={handlePause}
                      src="/audios/goodNoShare.mp3"
                    />
                  }
                >
                  Đúng rồi, những nội dung có thể khiến người khác không vui thì
                  chúng ta không nên chia sẻ!
                </CatConversation>
              )}
            ...
            <div className={`flex ${!isNoShare && "flex-col"} gap-5 mr-8`}>
              {!isNoShare ? (
                isLesson ? (
                  ...
                  <div className="flex flex-col items-center gap-5">
                    <span className="text-xl">
                      Bạn hãy đánh giá video này là an toàn hay không nhé!
                    </span>
                ...
                <div className="flex m-auto gap-5">
                  ...
                  <PrimaryButton
                    icon={<HomeFilled />}
                    onClick={() => router.replace(isLesson ? "/k1/hds02" : "/k1/hds05")}
                  >
                    Trang chủ
                  </PrimaryButton>
                </div>
